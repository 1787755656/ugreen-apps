...
# 轻课堂（LiteClass）上游【只发 Docker 镜像】—— 没有公开源码仓库、没有 Release、
# 没有 tag，镜像里的业务 JS 还是 javascript-obfuscator 混淆过的。
# 所以唯一的版本源是 Docker Hub 上的镜像 tag：
#
#   https://hub.docker.com/r/luodichanagn/liteclass/tags
#
...
#
# ⚠ 镜像里 server/package.json 的 version 恒为 1.0.0（上游没维护它），
#   别拿它当版本源 —— 那样每次上游发新版都探不到变化，永远不会重新构建。

REPO="luodichanagn/liteclass"
INPUT_VERSION="${1:-}"
...
if [ -z "$VERSION" ] || [ "$VERSION" = "null" ]; then
  echo "Failed to resolve version for liteclass" >&2
  exit 1
fi
