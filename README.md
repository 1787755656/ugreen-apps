...
包含：jellyfin（媒体服务器）、metatube（元数据刮削）、qbittorrent（Enhanced Edition）、natfrp（SakuraFrp 内网穿透客户端）、lucky（网络工具箱：DDNS/反代/端口转发等）、magicpush（多渠道消息推送平台）、adguardhome（DNS 广告过滤）。这些原本是桌面上各自独立的手工维护项目，现在合并成一个仓库统一自动化。
...
| 应用 | 上游 | 探测方式 | 备注 |
|---|---|---|---|
| jellyfin | `jellyfin/jellyfin`（server）+ `jellyfin/jellyfin-ffmpeg` | GitHub Releases API 查 server 版本；ffmpeg 单独从 `repo.jellyfin.org` 的 `latest-7.x` 目录抓最新，不单独追踪版本号 | server tarball URL: `repo.jellyfin.org/files/server/linux/stable/v{version}/{arch}/...` |
...
| adguardhome | `AdguardTeam/AdGuardHome` | GitHub Releases API，tag 格式 `vX.Y.Z` | 官方预编译二进制直接打包；管理界面 `--web-addr` 钉死 3000 端口 |

## 本地验证过什么（不需要真机、不需要GitHub仓库）

开发时已经用真实网络请求逐一跑通并验证：

- 6 个 `get-latest-version.sh` 都拿到了和当前手工打包版本完全一致的版本号
- 6 个 `build.sh` 都完整跑通下载/构建，`file` 命令确认产出的二进制架构正确（amd64/arm64 各测过）
...
1. **建仓库并推送**：
   ...
   git init
   git add .
   git commit -m "init: ugreen-apps monorepo with CI"
...
- **magicpush 和 adguardhome 都用 3000 端口**（两个 `project.yaml` 的 `port` 与各自 start.sh 里钉死的端口一致）：两个应用同时安装会端口冲突，装其中一个没问题。要同装需要改掉其中一个的端口（project.yaml 和 start.sh 一起改）。
